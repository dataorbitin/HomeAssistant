...
class EchoTests(unittest.TestCase):
    ...
    def post(self, payload):
        return self.client.post('/webhook/evolution', json=payload,
    ...
    def test_secret_required(self):
        self.assertEqual(self.client.post('/webhook/evolution', json=PAYLOAD).status_code, 401)
        self.assertEqual(self.calls, [])
    ...
    def test_query_token_echo(self):
        response = self.client.post('/webhook/evolution', json=PAYLOAD,
            params={'token': 's'*32})
        self.assertEqual(response.json()['echoed'], 1)

    def test_payload_limit(self):
        response = self.client.post('/webhook/evolution', content=b'x'*262145,
            headers={'x-webhook-secret': 's'*32})
        self.assertEqual(response.status_code, 413)
        self.assertEqual(self.calls, [])

    def test_missing_from_me_is_ignored(self):
        p = copy.deepcopy(PAYLOAD)
        del p['data']['key']['fromMe']
        self.assertEqual(self.post(p).json()['echoed'], 0)
        self.assertEqual(self.calls, [])
